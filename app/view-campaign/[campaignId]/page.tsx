...
export default function CampaignDetails({ params }: { params: { campaignId: number } }) {
  ...
  // Validate the campaignId and redirect if invalid
  ...
  // Redirect to home if campaign does not exist
    ...
    try {
      ...
      setTimeout(() => {
        ...
        toast({
          ...
          description: "The funds should reflect in the campaign shortly.",
        });
      }, 5000);
    } catch (error) {
  ...
  return (
    data && (
      <div className="container mx-auto px-4 pb-8">
        {isLoading && <Loader />}
        <h1 className='text-2xl font-semibold text-primary'>{data[1]}</h1>
        <div className="w-full flex md:flex-row flex-col mt-10 gap-8">
